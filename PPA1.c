...
void Initialize(void)
{
    ...
    // char displayString[SIZE] = "*COMPENG 2SH4 PPA1* "; 
    // char displayString[SIZE] = ">><<                "; 
...
void RunLogic(void)
{
    ...
    // [TODO]: Implement the above pseudocode logic
        ...
        { 
            ...
            {
                //leftward, from right to left
                direction = 1;
                // startPos = 0;
            }
            else if (cmd == '>')
            {
                //rightward, from left to right
                direction = 2;
            ...
            else if (cmd != '/' || cmd != '<' || cmd != '>')
            {
                ...
                if (ind > 17)
                {
                    ...
                    displayString[SIZE-4] = cmd;
                }
                else  
                {
                    for (i = SIZE-2; i >ind; i--)
                    {
                        displayString[i] = displayString[i-1];
                    }
                    displayString[ind] = cmd;
                    // printf("%d",ind);
    ...
    if (direction == 1)
        {
            ...
            for(i = 0; i <SIZE-1; i++)
            {
                ...
                MacUILib_printf("%c", displayString[(startPos+i) % (SIZE -1)]);

            }
            MacUILib_printf("\n====================\n");
            // MacUILib_printf("%d", ind);
    ...
    else if (direction ==2)
        {
            ...
            for(i = 0; i <SIZE - 1; i++)
            {
                ...
                temp = display2[SIZE - 2];

                for(j = SIZE-2; j > 0; j--)
                {   
                    display2[j] = display2[j-1];  
                }  
